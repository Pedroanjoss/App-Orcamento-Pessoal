...
class Bd {
    ...
    for (let i= 1; i<= id; i++) {
        ...
        // possibilidade de haver índices que foram pulados/removidos
    ...
    pesquisar(despesa){
        ...
        // ano
        if (despesa.ano != '') {despesasFiltradas = despesasFiltradas.filter(
            d => d.ano == despesa.ano) 
        }

        // mes
        if (despesa.mes != '') {despesasFiltradas = despesasFiltradas.filter(
            d => d.mes == despesa.mes) 
        }

        // dia

        if (despesa.dia != '') {despesasFiltradas = despesasFiltradas.filter(
            d => d.dia == despesa.dia) 
        }
        ...
        if (despesa.tipo != '') {despesasFiltradas = despesasFiltradas.filter(
        ...
        if (despesa.descricao != '') {despesasFiltradas = despesasFiltradas.filter(
        ...
        if (despesa.valor != '') {despesasFiltradas = despesasFiltradas.filter(
...
function cadastrarDespesa() {
  ...
  if (despesa.validarDados()) {
    ...
    document.getElementById('header').className = 'modal-header text-success'
    ...
    document.getElementById('modalTitulo').innerHTML = "Ouve um erro no registro"
...
function pesquisarDespesa() {
    ...
    carregaListaDespesas(despesa, true)
